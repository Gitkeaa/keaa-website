 ...
 *
 * A plain `<a download>` is IGNORED by browsers when the href is a different origin, which is
 * the case whenever VITE_API_DIRECT_ORIGIN points the console at a backend elsewhere, so it only
 * opens the file. This fetches the bytes WITH the admin cookie (credentials: 'include'), turns
 * them into a same-origin `blob:` URL, and downloads that, which browsers always honour. If the
 * fetch is blocked (no CORS on the file route, network), it falls back to opening the file so the
 * resume is never simply lost.
 *
